"""Atomic's Qt UI startup patches, applied on import.

Split out of helpers/__init__.py, 31 August 2026, so that importing
`helpers` does not require PyQt6.

The reason is the web shell: it imports helpers for their *data* -
storage, chapter_source, streams, torrent_engine, subtitles - none of
which touch Qt, and 61 of the 98 helpers are Qt-free. But a package's
__init__ runs whatever it likes, and this block imports twenty-five Qt UI
patch modules, so `from helpers import storage` pulled in the whole of Qt
and the Qt-free build died on `No module named 'PyQt6'` before its window
ever opened.

Nothing about the Qt app changes: __init__ still imports this module
unconditionally, and a patch that fails for any reason other than a
missing PyQt6 still raises exactly as it did. Only the absent-Qt case is
new, and it could not previously happen at all.
"""
...
# The scroll experiments that used to be listed here - motion_patch,
# poster_grid_motion_patch, high_refresh_live_pacing_patch,
# page_scroll_fixes_patch, hero_pages_live_scroll_patch,
# ultimate_scroll_patch, home_discover_interaction_patch and
# live_scroll_quantizer_patch - are **deleted**, not merely uninstalled
# (31 August 2026, his approval and CLAUDE.md rule 10). They were 90KB of
# superseded models that nothing imported, and every measurement of the
# scroll had to reason past them first. The two that remain listed above
# are player experiments and are left alone.
